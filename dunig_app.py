...
st.markdown("""
    ...
    .stButton>button:hover {
        ...
        background: linear-gradient(145deg, #e5c05b, #d4af37);
    }
...
if st.session_state.pagina == "inicio":
    ...
    st.markdown("<p style='text-align:center; color:white; font-family:Poppins;'>Exclusividad y Estilo en cada detalle</p>", unsafe_allow_html=True)
    st.write("---")
...
# --- PANEL DE GESTIÓN ---
    ...
    with st.expander("🖼️ CONFIGURAR PERFIL"):
        ...
        pago = st.text_area("Datos para el Pago")
        if st.button("Guardar Perfil"):
            st.success("✅ Perfil guardado (Simulado)")
...
# --- CENTRO COMERCIAL ---
    ...
    try:
        # Nota: Asegúrate de que la tabla 'perfiles_comercio' tenga datos
        ...
        if prods.data:
            for p in prods.data:
                with st.container():
                    ...
                    if st.button(f"🛒 AÑADIR", key=f"buy_{p['id']}"):
...
# --- PAGO ---
    ...
    # Lógica de resumen y WhatsApp...
    st.button("🔙 REGRESAR", on_click=navegar, args=("vitrina_personal", st.session_state.comercio_sel))
